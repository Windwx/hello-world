...
extern void vGPIODriver_SetOutput(int gpio, char *name, int level);
...
int spl_parse_schneider_image_header(const struct image_header *header)
{
	u32 header_size = sizeof(struct image_header);
	int result;
	
	/*! header checksum check */
	result = image_check_hcrc(header);
	if(result == 0){
	    debug("SPL header checksum error \r\n");
	    return 0;
	}
...
#else
	case BOOT_DEVICE_SPI:
		//printf("Load NAND Data after SPI_NOR Detect \r\n");
		spl_nand_load_image();
		break;
#endif
